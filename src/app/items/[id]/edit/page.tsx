    ...
    const fetchItemDetails = async () => {
      ...
      try {
        ...
        if (detailsRes.item) {
          ...
          // Check if user owns listing or is admin
          const ownerIdStr = (item.ownerId && typeof item.ownerId === 'object')
            ? (item.ownerId._id || item.ownerId.id)
            : item.ownerId;

          if (user && user.role !== 'admin' && ownerIdStr !== user.id) {
            setError('Unauthorized: You do not have permission to edit this listing.');
            setLoading(false);
            return;
          }
          ...
          const loadedUrls = item.imageUrls && item.imageUrls.length > 0 
            ? item.imageUrls 
    ...
    return (
      <div className="min-h-screen bg-[#090a0f] flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <span className="text-sm text-slate-400">Loading listing metadata...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow flex flex-col space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center space-x-2.5">
            ...
            <span>Edit Tech Listing</span>
          </h1>
          <p className="text-slate-400 text-xs mt-1.5">
            Modify specifications, pricing, or status details for this listing.
          </p>
        </div>
      </div>
      ...
      {success && (
        <div className="p-4 bg-emerald-500/10 border border-emerald-500/25 rounded-2xl text-emerald-400 text-sm font-semibold flex items-center space-x-3 justify-center animate-pulse">
          <span>🎉 Listing successfully updated! Redirecting to dashboard...</span>
        </div>
      )}
      ...
      {/* Only display form if there's no auth/permission block error */}
      {!success && (!error || error.indexOf('Unauthorized') === -1) && (
        <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-8 border border-white/5 space-y-6 animate-fadeIn">
          ...
          <div className="pt-6 border-t border-white/5 flex justify-end">
            ...
            >
              {submitting ? (
                ...
                <span>Save Listing Modifications</span>
              )}
            </button>
          </div>
        </form>
