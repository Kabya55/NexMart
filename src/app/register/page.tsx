...
export default function RegisterPage() {
  ...
  // States for error and independent loading management
  const [error, setError] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Using FormData for uncontrolled inputs (default form behavior)
    ...
    try {
      await register(name, email, password);
      formRef.current?.reset(); // Clear form after successful registration
    } catch (err: any) {
      setError(err.message || 'Registration failed. Email might already be taken.');
  ...
  const isAnyLoading = isRegistering || socialLoading !== null;
